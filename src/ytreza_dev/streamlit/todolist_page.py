...
from ytreza_dev.features.final_version_perfected.adapter.for_demo import TaskInformationRepositoryForDemo
...
def provide_dependencies() -> PyqureMemory:
    ...
    provide(FVP_REPOSITORY_KEY, FvpRepositoryFromJson(file_path=Path(env_reader.read(key="FVP_JSON_PATH"))))
    ...
    provide(TASK_INFORMATION_REPOSITORY_KEY, TaskInformationRepositoryFromJson(json_path=Path(env_reader.read(key="TASK_REPOSITORY_JSON_PATH"))))
