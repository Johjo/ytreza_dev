...
class DoNext:
    def __init__(self, task_repository: FvpRepositoryPort):
        self._task_repository = task_repository

    def execute(self, updated_key: str) -> None:
        before: list[TaskBase] = self._task_repository.all_tasks()

        self._task_repository.save([self._update_first_task(before)] + [self.update_task(task, updated_key) for task in before[1:]])
    ...
    @staticmethod
    def update_task(task: TaskBase, updated_key: str) -> TaskBase:
