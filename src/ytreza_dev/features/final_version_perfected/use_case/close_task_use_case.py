...
class CloseTaskUseCase:
    def __init__(self, task_repository: FvpRepositoryPort, external_todolist: ExternalTodolistPort) -> None:
        self._external_todolist = external_todolist
        self._task_repository = task_repository

    def execute(self, key: str) -> None:
        after, closed_task, before = self._extract_before_and_task_and_after(self._task_repository.all_tasks(), key)
        ...
        self._external_todolist.close_task(url=key, task_id=closed_task.id)
    ...
    @staticmethod
    def _extract_before_and_task_and_after(before: list[TaskBase], key: str) -> tuple[list[TaskBase], TaskBase,list[TaskBase]]:
        after = []
        # on garde toutes les tâches avant la tâche à fermer
        i = 0
        while before[i].id != key:
